...
- **⚠️ TODO/unverified: `POWER_STATE_MAP` in `const.py` looks wrong** — noticed while researching `conn` above. MeshCentral's own `powerStateStrings` comment in `meshcentral.js` maps the `pwr` field as `0=Unknown, 1=S0 power on, 2=S1 Sleep, 3=S2 Sleep, 4=S3 Sleep, 5=S4 Hibernate, 6=S5 Soft-Off, 7=Present, 8=Off`, but our `POWER_STATE_MAP` currently has `0:"off", 1:"on", 2:"sleep", 3:"hibernate", 4:"soft_off", 5:"cycle"` — different values *and* only 6 entries instead of 9. Haven't touched this since it's unrelated to #26 and untested — worth its own issue/verification pass before fixing
